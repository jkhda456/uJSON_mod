...
Type
  ...
  public
    ...
    *)
    ...
    function getByIndex (index: Integer) : TZAbstractObject;
    function getKeyByIndex (index: Integer) : string;
    function getSlient (key : string) : TZAbstractObject; // ¿¡·¯ ¾È³ª´Â Get
    ...
    function getStringSlientByIndex (index: Integer) : String;
    procedure setKeyByIndex (index: Integer; key: String);
    procedure setValueByKey (key: String; const value: TZAbstractObject);
    procedure setValueByIndex (index: Integer; const value: TZAbstractObject);
    ...
    *)
    ...
    function putint64(key : string; value : int64): TJSONObject; overload;
    ...
    *)
    ...
    class function quote (s : widestring): string;
  ...
  public
    ...
    function putint64( value : int64) : TJSONArray;   overload ;
  ...
  _Double = class (_Number)
     ...
     function intValue : int64;  override;
     function toString () : string ; override;
  ...
  _String = class (TZAbstractObject)
   ...
   procedure OverWrite(AValue: String);
  private
     fvalue : string;
  end;
...
begin
  if System.length(self.mySource) <= 0 then
  begin
     result := false;
     exit;
  end;
  result := self.myIndex <= System.length(self.mySource); // +1 // jkh note
end;
        ...
        if ( ((b >= '0') and (b <= '9')) or (b = '.')
            ...
            if (b = '0') then begin
                if ( (System.length(s) > 2) and
                    ...
                    If Pos('.', s) <= 0 Then // . µé¾î°£°Ç Double ·Î ¹Ù·Î °¡µµ·Ï ÇÑ´Ù. by jkh
                    try
            ...
            If Pos('.', s) <= 0 Then // . µé¾î°£°Ç Double ·Î ¹Ù·Î °¡µµ·Ï ÇÑ´Ù. by jkh
            try
                // java ÀÇ big int Å¸ÀÔÀº ¹è¿­ ÇüÅÂ·Î ¹«½ÄÇÏ°Ô ±æ°Ô µé¾î¿Ã°Å´Ù.
                If Length(s) < 23 Then
                   result := _Integer.create(s)
                Else
                   result := CNULL; // ¸»µµ ¾ÈµÇ°Ô Å©´Ù. ÀÌ°Ç ÀÏ´Ü ¿¹¿ÜÃ³¸® ÀÌÀü¿¡ ÀÌ·¸°Ô ÇÊÅÍ¸µ ÇÑ´Ù.
                exit;
            Except
                    on e:Exception do begin
                      ///* Ignore the error */
...
begin
 result := ParseException.create (_message + toString()+' ParseException : '
...
begin
  ...
  try
    create (token);
  finally // add by jkh Finally ¸¦ ³Ö¾î¼­ ¸Þ¸ð¸® ´©¼ö°¡ »ý±âÁö ¾Êµµ·Ï Çß´Ù.
    token.free;
  end;
end;
...
function TJSONObject.getByIndex (index: Integer) : TZAbstractObject;
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ÀÐ¾îµéÀÌ´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
  Result := nil;

  If (index < 0) or (myHashMap.Count <= index) Then Exit;

  result :=  TZAbstractObject (myHashMap.Objects [index]);
end;

function TJSONObject.getKeyByIndex (index: Integer) : string;
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ÀÐ¾îµéÀÌ´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
  Result := '';
...
(**
     ...
     *)
function TJSONObject.getInt(key: string): int64;
var
  o : TZAbstractObject;
begin
        o := get(key);
...
var
 i : integer;
begin
  // Key ´Â »ç¿ë½Ã ¹Ýµå½Ã Free ÇØÁà¾ß ÇÔ.
...
function TJSONObject.putint64(key: string; value: int64): TJSONObject;
begin
   put(key, _Integer.create(value));
   result := self;
end;
...
(**
     ...
     *)
class function TJSONObject.quote(s: widestring): string;
var
   b,c : widechar;
   i, len : integer;
   sb, t : string;
const
  ...
  function CheckConversion(const AChar: WideChar): Boolean;
  var
    LoopVar: Integer;
  begin
    Result := True;
  ...
  // Modify by jkh 
        ...
        for i := 1 to len do begin
            ...
            #13:  begin
                ...
                // jkh note.. ÇÑ±Û ¹× Ã³¸® ºÒ°¡ ¹®ÀÚ¿­ÀÇ ÀÚµ¿ ÀÎÄÚµù ·çÆ¾, ÀÌÂÊÀ» »©°í ½ÍÀ¸¸é »¬°Í.
                // (!) 
                // if (not (c in NoConversion)) then begin
                if CheckConversion(c) then begin
                    sb := sb + '\u' + IntToHex(Word(c), 4);
...
function TJSONObject.getSlient(key: string): TZAbstractObject;
var
 o : TZAbstractObject;
begin
  o := opt(key);
...
function TJSONObject.getStringSlient(key: string): string;
var
  temp: TZAbstractObject;
begin
  result := '';
  temp := getSlient(key);
...
function TJSONObject.getJSONValueString(key: string): string;
var
  temp: TZAbstractObject;
begin
  result := '';
  ...
  // ´Ù½Ã JSON String À¸·Î ¹ÝÈ¯½ÃÅ²´Ù.
  result := TJSONObject.valueToString(temp);
end;

function TJSONObject.getIndexByKey(key: String): Integer;
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ¾²´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
  result := -1;
  if (key = '') then exit;

  result := myHashMap.IndexOf(key);
end;

procedure TJSONObject.setKeyByIndex (index: Integer; key: String);
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ¾²´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
  If (index < 0) or (myHashMap.Count <= index) Then Exit;
...
procedure TJSONObject.setValueByKey(key: String;
  const value: TZAbstractObject);
var
  index: Integer;
begin
  // add by all jkh
  ...
  index := myHashMap.IndexOf(key);
  If (index < 0) or (myHashMap.Count <= index) Then Exit;

  myHashMap.Objects[index].Free;
  myHashMap.Objects[index] := value;
end;

procedure TJSONObject.setValueByIndex(index: Integer;
  const value: TZAbstractObject);
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ¾²´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
...
procedure TJSONObject.rewrite(s: string);
var
  x : JSOnTokener;
  c : char;
  key : string;
...
function TJSONObject.getStringSlientByIndex(index: Integer): String;
var
  findObject: TZAbstractObject;
begin
  // add by all jkh
  // ÀÎµ¦½º·Î ¹Ù·Î ÀÐ¾îµéÀÌ´Â ±â´ÉÀÌ ¾ø¾î¼­ ¸¸µé¾îµÒ. ¿ø·¡ÀÇ JSON¿£ ¾øÁö¸¸ ³­ ÀÌ°Ô ÁÁ´Ù.
  Result := '';
  ...
  findObject := TZAbstractObject (myHashMap.Objects [index]);
  Result := findObject.toString;
end;

function TJSONObject.getBooleanSlient(key: string; default: boolean): boolean;
var
  temp: TZAbstractObject;
begin
  result := default;
  ...
  if (temp.equals(_Boolean._FALSE) or
          ((temp is _String) and
...
procedure _String.OverWrite(AValue: String);
begin
  fvalue := AValue;
end;
...
begin
   inherited createFmt(_message+#10#13' error no caracter : %d',[index]);
end;
...
function _Integer.intValue: int64;
begin
  result := fvalue;
end;
...
class function _Integer.parseInt(s: string; i: int64): int64;
begin
...
class function _Integer.parseInt(s: _String): int64;
begin
...
function _Double.intValue: int64;
begin
  result := trunc (fvalue);
end;
...
procedure TJSONArray.rewrite(s: string);
var
  x : JSOnTokener;
  i : integer;
  obj : TObject;
...
(**
     ...
     *  if the value cannot be converted to a number.)
     *)
function TJSONArray.getInt(index: integer): int64;
var
  o : TZAbstractObject;
begin
  o := get(index);
...
function TJSONArray.getChar(index: integer): char;
var
  o : TZAbstractObject;
  ch: String;
begin
  o := get(index);
  if (o is _Number) then begin
    result := Char(_Number(o).intValue());
  end else begin
    ch := o.toString();
    If System.Length(ch) >= 1 then
      result := ch[1]
    Else
      result := #0;
  end;
...
procedure TJSONArray.setValueByIndex(index: Integer;
  const value: TZAbstractObject);
begin
  If ((index < 0) or (index >= length()) ) then
     Exit;

  TZAbstractObject(myArrayList[index]).Free;
...
function TJSONArray.putint64(value: int64): TJSONArray;
begin
  put(_Integer.create(value));
  result := self;
end;
...
function TJSONArray.putint64(index: integer; value: int64): TJSONArray;
begin
  put(index, _Integer.create(value));
  result := self;
end;
